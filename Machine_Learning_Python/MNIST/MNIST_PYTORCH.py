...
class CNN(nn.Module):
    ...
    def forward(self, x):
        x = self.pool(
            F.relu(self.conv1(x))
        )  # input:28x28; after conv:24x24 after pool: 12x12
        x = self.pool(
            F.relu(self.conv2(x))
        )  # input:12x12; after conv:9x9 after pool: 4x4
        x = x.view(-1, 16 * 4 * 4)
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return x
    ...
    for epoch in range(n_epochs):
        for i, (img, labels) in enumerate(train_loader):
            ...
            if (i + 1) % int(len(train_loader) / 10) == 0:
                print(
                    f"epoch: {epoch+1}/{n_epochs}, step {i+1}/{len(train_loader)}, loss: {loss.item():.4f}"
                )
                writer.add_scalar(
                    "training loss",
                    mean_loss / 100,
                ...
                writer.add_scalar(
                    "accuracy", epoch_acc / 100, epoch * len(train_loader) + i
                )
                mean_loss = 0.0
                epoch_acc = 0
